...
if (isset($_POST["delete_save"])) {
  ...
  // Delete the record
  $delete_query = "DELETE FROM archived_inventory WHERE ID = ?";
  $stmt = $conn->prepare($delete_query);
  $stmt->bind_param("i", $id);

  if ($stmt->execute()) {
      // Insert into archived_discounts
      $insert_query = "INSERT INTO db_inventory (category, item, cases, pack, kgs, pcs, gms, threshold, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
      $stmt = $conn->prepare($insert_query);
      $stmt->bind_param("sssssssss", $edit_category, $edit_item, $edit_case, $edit_pack, $edit_kgs, $edit_pcs, $edit_gms, $edit_threshold, $edit_status);
...
$sql = 'SELECT * FROM archived_inventory';
...
<?php require_once '../navbar/header.php';?>
  ...
  <!-- Alerts will be inserted here -->
    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item active">Archived / Inventory <i class="bi bi-archive"></i></li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
          ...
          <form class="row g-3" id="editInventoryForm" method="POST">
            ...
            <div class="col-md-12">
              <div class="form-floating">
                <input type="text" class="form-control" id="edit_category" name="edit_category" placeholder="Category" readonly>
                <label for="edit_category">Category</label>
              </div>
            </div>
            <div class="col-md-12">
              <div class="form-floating">
                <input type="text" class="form-control" id="edit_item" name="edit_item" placeholder="Item Name (ex: Butterscotch sauce/2L)" readonly>
                <label for="edit_item">Item Name (ex: Butterscotch sauce/2L)</label>
              </div>
            </div>
            <div class="col-md-6">
              <div class="form-floating">
                <input type="number" class="form-control" id="edit_case" name="edit_case" placeholder="Case (ex: 1)" min="0" readonly>
                <label for="edit_case">Case (ex: 1)</label>
              </div>
            </div>
            <div class="col-md-6">
              <div class="form-floating">
                <input type="number" class="form-control" id="edit_pack" name="edit_pack" placeholder="Pack / Inner (ex: 1)" min="0" readonly>
                <label for="edit_pack">Pack / Inner (ex: 1)</label>
              </div>
            </div>
            <div class="col-md-6">
              <div class="form-floating">
                <input type="number" class="form-control" id="edit_kgs" name="edit_kgs" placeholder="KG (ex: 1)" min="0" readonly>
                <label for="edit_kgs">KG (ex: 1)</label>
              </div>
            </div>
            <div class="col-md-6">
              <div class="form-floating">
                <input type="number" class="form-control" id="edit_pcs" name="edit_pcs" placeholder="PCS (ex: 1)" min="0" readonly>
                <label for="edit_pcs">PCS (ex: 1)</label>
              </div>
            </div>
            <div class="col-md-6">
              <div class="form-floating">
                <input type="number" class="form-control" id="edit_gms" name="edit_gms" placeholder="GMS (ex: 1)" min="0" readonly>
                <label for="edit_gms">GMS (ex: 1)</label>
              </div>
            </div>
            <div class="col-md-6">
              <div class="form-floating">
                <input type="number" class="form-control" id="edit_threshold" name="edit_threshold" placeholder="GMS (ex: 1)" min="0" readonly>

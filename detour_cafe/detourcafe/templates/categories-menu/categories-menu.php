...
if (isset($_POST["save"])) {
    ...
    $save_query = "INSERT INTO db_menu_category (category, type) VALUES ('$category', '$type')";
...
if (isset($_POST["update_save"])) {
    ...
    // SQL query for update
    $update_query = "UPDATE db_menu_category SET 
                    category = '$category', 
                    type = '$type'
                    WHERE ID = '$id'";
...
if (isset($_POST["delete_save"])) {
    // Retrieve data from POST
    $id = $_POST["edit_id"];
    $delete_query = "DELETE FROM db_menu_category WHERE ID = '$id'";
...
if (isset($_POST["delete_save"])) {
  ...
  // Delete the record
  $delete_query = "DELETE FROM db_menu_category WHERE ID = ?";
  $stmt = $conn->prepare($delete_query);
  $stmt->bind_param("i", $id);

  if ($stmt->execute()) {
      // Insert into archived_discounts
      $insert_query = "INSERT INTO archived_menu_category (category, type, archive_date) VALUES (?, ?, ?)";
...
$sql = 'SELECT * FROM db_menu_category';
...
<?php require_once '../navbar/header.php';?>
  ...
  <!-- Alerts will be inserted here -->
    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          <li class="breadcrumb-item active">Management / Menu <i class="bi bi-card-list"></i></li>
        </ol>
      </nav>
    </div><!-- End Page Title -->
    ...
    <div class="modal-dialog modal-dialog-centered">
        ...
        <form id="addCategoryForm" method="post">
            ...
            <div class="row g-3">
                ...
                <div class="col-md-12">
                <div class="form-floating">
                    <input type="text" name="category" class="form-control" id="category" placeholder="Category (Example: Signature Drinks)" required>
                    <label for="category">Category (Example: Signature Drinks)</label>
                </div>
                </div>
            </div>
            </div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form method = "post" id="editCategoryForm" class="row g-3">
            ...
            <div class="col-md-8">
                <div class="form-floating">
                    <input type="text" name="edit_category" class="form-control" id="edit_category" placeholder="Category (Example: Signature Drinks)" required>
